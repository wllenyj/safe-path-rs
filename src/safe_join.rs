...
/// filesystem) after this function has returned. You may use [crate::SafePathBuf] to protect from
/// such TOCTOU attacks.
...
///
/// The `safe_join()` function assumes `root` exists and is an absolute path. It safely joins the
...
///
/// # Security
/// On success return, the `safe_join()` function guarantees that:
...
mod tests {
    ...
    fn test_scoped_resolve_symlink() {
        ...
        fs::symlink(
            "../usr/bin/../../../bin/ls",
            rootfs_path.join("symlink_dir/5"),
        )
        .unwrap();
        let tests = [TestData {
            name: "symlink try to escape",
            rootfs: rootfs_path,
            unsafe_path: "symlink_dir/5",
            result: "bin/ls",
        }];
        exec_tests(&tests);
        ...
        safe_join(rootfs_path, "endpoint_a").unwrap_err();
    }
}
